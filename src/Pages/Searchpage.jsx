...
import { Filter, Grid, List, MapPin, Phone, Search, Tag, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Searchpage = () => {
  ...
  const fetchSearchedBusinesses = async (q = '', category = '', location = '') => {
    try {
      setLoading(true);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="bg-yellowCustom py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-6">
              <h1 className="text-5xl font-bold text-black mb-2">Search Local Businesses</h1>
              <p className="text-lg text-black">Find local services, shops, and more near you</p>
            </div>

            {/* ✅ Search Bar */}
            <div className="max-w-4xl mx-auto flex gap-4 flex-col md:flex-row border border-gray-100 p-6 rounded-md bg-white shadow">
        ...
        {/* ✅ Main Layout: Sidebar + Business List */}
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  className={`p-2 flex-1 rounded-lg ${viewMode === 'grid' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
                  <Grid className="w-5 h-5 mx-auto" />
                </button>
                <button
                  onClick={() => setViewMode('list')}
                  className={`p-2 flex-1 rounded-lg ${viewMode === 'list' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
                >
          ...
          <div className="lg:w-3/4">
            {loading ? (
              ...
              <div className="bg-white rounded-lg shadow-sm p-12 text-center">
                ...
                <p className="text-gray-600">Try refining your search or filters</p>
              </div>
            ) : (
              <>
                <div className={`grid gap-6 ${viewMode === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "grid-cols-1"}`}>
                  {currentBusinesses.map((business) => (
                    <div
                      key={business._id}
                      className={`p-4 bg-white border border-gray-200 rounded-lg shadow-sm transition-transform duration-200 hover:scale-[1.02] flex ${viewMode === 'grid' ? 'flex-col' : 'flex-col sm:flex-row'} items-start`}
                    >
                      <img
                        src={business.images?.[0] || '/placeholder.jpg'}
                        alt={business.name}
                        className={`${viewMode === 'grid' ? 'w-full h-48' : 'w-full sm:w-48 h-48'} object-cover rounded-md mb-4 sm:mb-0 sm:mr-4`}
                      />
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        ...
                        <div className="mt-4 space-y-2 text-center sm:text-left">
                          <p className="text-sm text-gray-600 flex items-center justify-center sm:justify-start">
                            <MapPin className="w-4 h-4 mr-1 text-gray-500" /> {business.address}
                          </p>
                          <p className="text-sm text-gray-600 flex items-center justify-center sm:justify-start">
                            <Phone className="w-4 h-4 mr-1 text-gray-500" /> {business.phone}
                          </p>
                        </div>
                        ...
                        <button
                          onClick={() =>
                            navigate(`/viewdetail/${business._id}`, { state: { business } })
                          }
